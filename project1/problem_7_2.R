rm(list = ls())
library("igraph")
setwd("/Users/yunqiguo/Document/EE232/Proj1/")
folder = "gplus/"
...
for(i in 1:length(egoNodeIds)){
  ...
  write(paste(egoNodeId," ","Number of communties",":",max(comm$membership)," ","Number of circles",":",length(circles),sep=""), file="p2_walktrap.txt", append=TRUE)
  for(j in 1:max(comm$membership)){ # no of comm of a node
    ...
    write(paste('Community: ',j,sep=""), file="p2_walktrap.txt", append=TRUE)
    write(paste("Overlap Ratio (Communites):", percentage_comm, ",Overlap Ratio (Circles):", percentage_circle,sep=""), file="p2_walktrap.txt", append=TRUE)
  }
  write('End of information for this user', file="p2_walktrap.txt", append=TRUE)
